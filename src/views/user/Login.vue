...
<!--          </a-form-item>-->
          <a-form-item>
            ...
            >
              <a-select-option v-for="college in getCollegesInfo()" :key="college">
                <a-icon slot="prefix" type="user" :style="{ color: 'rgba(0,0,0,.25)' }"/>
                {{ college }}
              </a-select-option>
            </a-select>
...
export default {
  ...
  methods: {
    ...
    getCollegesInfo: function () {
      const colleges = []
      const allColleges = this.$store.state.allColleges
      for (var department in allColleges) {
        if (allColleges[department].hasOwnProperty('单位性质号')) {
          if (allColleges[department]['单位性质号'] === '1') {
            colleges.push(department)
          }
        }
      }
      return colleges
